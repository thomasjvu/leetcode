...
const differenceOfSum = (nums) => {
    ...
    // get digit sum by concatenating each array element into a single string
    // then loop through each character in string and add numeric value to digitSum
    let digitSum = 0
    let digits = ''

    for (let i = 0; i < nums.length; i++) {
        digits += nums[i].toString()
    }
